    ...
    public partial class FormPrintBirthdays
	{
		...
		private void printBirthdayDoc_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
		{
			...
			for (int value = 0; value <= min; value++) {
                ...
                string niceFormatBirthday = "N/a";
                string niceFormatDays = "N/a";
                if (theListener.Birthday.HasValue)
                {
                    // Format nicely.
                    int monthInt = theListener.Birthday.Value.Month;
                    int dayInt = theListener.Birthday.Value.Day;
                    niceFormatBirthday = dayInt + " " + DateAndTime.MonthName(monthInt);
                    
				    // Get the day of the week the birthday has this year not the year of birth.
				    DateTime time = new DateTime(DateTime.Now.Year, monthInt, dayInt);
                    niceFormatDays = time.ToString("dddd");
                }

                g.DrawString(niceFormatBirthday, reportFontSmallBold, Brushes.Black, 460, 240 + (70 * value));
                g.DrawString(niceFormatDays, reportFontSmallBold, Brushes.Black, 650, 240 + (70 * value));
		...
		{
            theListeners = theListeners.OrderBy(x => x.Birthday.Value.AddYears(DateTime.Today.Year - x.Birthday.Value.Year)).ToList();
		}
